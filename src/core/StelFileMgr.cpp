...
void StelFileMgr::init()
{
	...
	else
	{
        ...
        QFileInfo installLocation("/usr/share/stellarium");
        QFileInfo checkFile("/usr/share/stellarium/data/ssystem.ini");
...
#endif
        //installDir = installLocation.filePath(); //added
        if (checkFile.exists())
		{
...
{
#ifdef ENABLE_NLS
    QFileInfo localePath = QFileInfo(getInstallationDir() + "/locale/");

		...
		)
{
	...
	double delt, ft, x2li, x2omi, xl, xldot , xnddt, xndt, xomi;
	...
	static const double rptim = 4.37526908801129966e-3; // this equates to 7.29211514668855e-5 rad/sec
	static const double stepp =    720.0;
	static const double stepn =   -720.0;
	...
	for (;;)
	{
		const double cosxli = cos(xli);
		const double sinxli = sin(xli);

		/* ------------------- dot terms calculated ------------- */
		/* ----------- near - synchronous resonance terms ------- */
		if (irez != 2)
		{
			xndt  = del1 * (sinxli * cos(fasx2) - cosxli * sin(fasx2)) + del2 * (2.0 * sinxli * cosxli * cos(2.0*fasx4) - (1. - 2.*sinxli * sinxli) * sin(2.0*fasx4)) +
					del3 * ((3.0*sinxli-4.0*sinxli*sinxli*sinxli)*cos(3.0*fasx6) - (-3.0*cosxli+4.0*cosxli*cosxli*cosxli)*sin(3.0*fasx6));
			xldot = xni + xfact;
			xnddt = del1 * (cosxli*cos(fasx2)+sinxli*sin(fasx2)) +
					2.0 * del2 * ((1.0-2.0*sinxli*sinxli)*cos(2.0*fasx4) + 2.0*sinxli*cosxli*sin(2.0*fasx4)) +
					3.0 * del3 * ((-3.0*cosxli+4.0*cosxli*cosxli*cosxli)*cos(3.0*fasx6) + (3.0*sinxli-4.0*sinxli*sinxli*sinxli)*sin(3.0*fasx6));
			xnddt = xnddt * xldot;
		}
		else
		{

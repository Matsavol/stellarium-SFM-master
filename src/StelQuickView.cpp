...
StelQuickView::StelQuickView() : stelApp(NULL), nightMode(false), quitRequested(false)
{
    ...
    setSource(QUrl("/usr/share/stellarium/qml/Splash.qml")); // was "qrc:/qml/Splash.qml
...
{
	globalConf = conf;
#if !(defined(Q_OS_ANDROID) || defined(Q_OS_IOS))  //added
    int width = conf->value("video/screen_w", 480).toInt(); //added
    int height = conf->value("video/screen_h", 700).toInt(); //added
    resize(width, height); //added
#endif //added
...
//#if defined(Q_OS_ANDROID) || defined(Q_OS_IOS)  //removed
    show();
//#else //removed
    //int width = conf->value("video/screen_w", 480).toInt();  //removed
    //int height = conf->value("video/screen_h", 700).toInt(); //removed
    //resize(width, height); //removed
    //show(); //removed
//#endif //removed
	timer->start();
...
#else
    return screen()->physicalDotsPerInch()/160.f;  //edited from 96.f
#endif
}
...
{
    setSource(QUrl("/usr/share/stellarium/qml/main.qml")); // was "qrc:/qml/main.qml"
#ifdef Q_OS_ANDROID
